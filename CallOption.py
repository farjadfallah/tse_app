...
class CallOption(Option):
    def __init__(self,_op_code, _ticker, _strike, _u_asset, _maturity_date, _days_till_maturity, _best_kharid, _best_foroosh, _volume, _last_price):
        super().__init__(_op_code, _ticker, _strike, _u_asset, _maturity_date, _days_till_maturity, _best_kharid, _best_foroosh, _volume, _last_price)
    ...
    def get_value_at_price(self, final_price, had_vajh_tazmin= False, sarkhat_or_latest= 'sarkhat'):
         if(not had_vajh_tazmin):
            ...
            return (-1) * (self.vajh_tazmin + previous_frozen_premium) + max(final_price - self.strike, 0) 
